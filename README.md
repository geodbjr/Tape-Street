...
In v1.2.7-M, we implement a memory-hard architecture that prioritizes hardware saturation over algorithmic complexity.
    ...
    • Thermal Deterrence: Parallelism is hard coded to P=8. To handshake with a Tape Street vault is to force a multi-core thermal event. If the cooling fans aren't spinning, the vault isn't winning.
